...
async def analyze_standard_criteria(standard_id: str, institution_id: str, evidence_ids: Optional[List[str]] = None, force_reanalyze: bool = False):
    ...
    for _ in [1]:
        try:
            ...
            # 6. Bulk insert mappings (better performance than loop per record)
            if mappings_to_create:
                try:
                    # Use create_many for bulk insert when available
                    await db.criteriamapping.create_many(data=mappings_to_create)
                    logger.info(f"✅ Bulk inserted {len(mappings_to_create)} CriteriaMapping records for standard {standard_id}")
                except Exception as bulk_err:
                    logger.warning(f"Bulk insert not supported or failed, falling back to single inserts: {bulk_err}")
                    for mapping in mappings_to_create:
                        await db.criteriamapping.create(data=mapping)
                    logger.info(f"✅ Saved {len(mappings_to_create)} CriteriaMapping records for standard {standard_id} via fallback")
            else:
