...
function AnalyticsContent() {
  ...
  const distributionColors = ["var(--primary)", "var(--status-info)", "var(--status-success)", "var(--status-warning)"]
              ...
              <div className="flex-1 space-y-6 flex flex-col justify-center">
                {distributionData.length > 0 ? (
                    ...
                    <div key={i} className="group">
                      <div className="flex justify-between items-end mb-2">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-tight group-hover:text-foreground transition-colors truncate max-w-[140px]">{item.label}</span>
                        </div>
                  ...
                  <div className="flex flex-col items-center justify-center text-center opacity-40">
                    ...
                    <p className="text-[10px] uppercase font-bold text-muted-foreground">No Distribution Data</p>
                  </div>
                )}
              </div>
              <div className="mt-8 pt-6 border-t border-[var(--border-subtle)] flex justify-between items-center">
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Sync Health: {reports ? "100%" : "—"}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {reports && reports.length > 0 ? (
                ...
                <div className="col-span-full py-12 text-center glass-panel rounded-[32px] border-dashed border-[var(--border-subtle)]">
                  <p className="text-muted-foreground text-sm font-medium">No intelligence briefings generated yet.</p>
                  <button
                    onClick={() => router.push('/platform/gap-analysis')}
                    className="mt-4 text-xs font-bold text-primary uppercase tracking-widest hover:text-primary/80 pointer-cursor"
                  >
