      ...
      <SheetContent side="right" className="inset-y-4 right-4 h-auto w-[min(520px,calc(100vw-32px))] overflow-y-auto rounded-3xl border border-[var(--glass-border)] bg-background/96 p-0 text-foreground shadow-2xl backdrop-blur-xl">
        ...
        <div className="space-y-6 p-6">
          {isLoading ? (
            ...
            <>
              <section>
                <h3 className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-primary">Summary</h3>
              ...
              <section>
                <h3 className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-primary">Top 3 Problems</h3>
                <div className="space-y-2">
                  {explanation.topProblems.map((problem, index) => (
                    <div key={problem} className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.05] px-4 py-3 text-sm leading-relaxed text-muted-foreground">
                      <span className="me-2 font-mono font-bold text-amber-400">{index + 1}</span>
                      {problem}
                    </div>
                  ))}
                </div>
              ...
              <section>
                <h3 className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-primary">Impact</h3>
              ...
              <section>
                <h3 className="mb-3 text-xs font-bold uppercase tracking-[0.18em] text-primary">Recommended Plan</h3>
                <div className="space-y-2">
              ...
              <section>
                <div className="mb-3 flex items-center justify-between gap-3">
                  <h3 className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Go Fix This</h3>
                  <span className="rounded-full border border-[var(--glass-border)] bg-[var(--glass-soft-bg)] px-2 py-1 text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground">
                ...
                <div className="flex flex-wrap gap-2">
                  {explanation.links.map((link) => (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-xl border border-[var(--glass-border)] bg-[var(--glass-soft-bg)] px-3 py-2 text-xs font-bold uppercase tracking-[0.14em] text-foreground transition-colors hover:bg-[var(--glass-strong-bg)]"
                    >
                      {link.label}
...
function AnalyticsContent() {
  ...
  const handleExportReport = async () => {
    ...
    try {
      ...
      await api.downloadAnalyticsReportPdf(periodDays)
  ...
  /* ════════════════════════════════════════════════════════════
     ...
     ════════════════════════════════════════════════════════════ */
  return (
    <div className={cn("animate-fade-in-up pb-20 platform-container-wide mx-auto", isArabic && "font-arabic")} dir={isArabic ? "rtl" : "ltr"}>
      <HorusExplainSheet
        open={explainOpen}
      ...
      <header className="mb-7 pt-2">
        <div className="premium-surface relative overflow-hidden p-5 sm:p-6 lg:p-7">
          ...
          <div className="relative z-10 flex flex-col gap-4 lg:gap-5">
            <div className={cn("min-w-0 max-w-3xl space-y-2", isArabic && "text-right")}>
              ...
              <p className="max-w-2xl text-sm sm:text-base text-foreground/78 leading-relaxed">
                {executiveSummary}
              </p>
            </div>
            ...
            <div
              className={cn(
                "flex flex-col gap-3 pt-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-2 sm:gap-y-2",
              )}
            >
              <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:flex-1 sm:gap-2">
                <div className="flex max-w-full flex-nowrap items-stretch gap-1 overflow-x-auto rounded-2xl border border-[var(--glass-border-subtle)] bg-[var(--glass-soft-bg)] p-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                  {PERIOD_OPTIONS.map((option) => (
                    <button
                      ...
                      className={cn(
                        "shrink-0 rounded-xl px-3.5 py-2 text-xs font-semibold uppercase tracking-wide transition-colors",
                ...
                <div className="hidden h-8 w-px shrink-0 bg-[var(--glass-border-subtle)] sm:block" aria-hidden />
                ...
                <div className="flex items-center gap-2.5">
                  <button
                    ...
                    className={cn(
                      "group inline-flex h-10 min-h-[40px] shrink-0 items-center justify-center gap-2 rounded-2xl border border-[var(--glass-border-subtle)] bg-[var(--glass-soft-bg)] px-3.5",
                      "text-xs font-semibold uppercase tracking-wide text-foreground/90 transition-colors hover:bg-white/12 hover:text-foreground",
                    )}
                  >
                    <RefreshCw
                      className={cn(
                  ...
                  {lastUpdated && (
                    <span className="hidden text-xs font-medium text-foreground/62 tabular-nums sm:inline whitespace-nowrap">
              ...
              <div className="flex flex-wrap items-stretch gap-2 sm:ms-auto sm:justify-end sm:ps-2">
                <button
                  ...
                  className={cn(
                    "inline-flex h-10 min-h-[40px] items-center justify-center gap-2 rounded-2xl border border-primary/30 bg-primary/10 px-4",
                    "text-xs font-semibold uppercase tracking-wide text-primary transition-colors hover:bg-primary/20",
                ...
                <button
                  ...
                  className={cn(
                    ...
                    "text-xs font-semibold uppercase tracking-wide text-primary-foreground shadow-md shadow-primary/30",
              ...
              {lastUpdated && (
                <p className="w-full text-xs font-medium text-foreground/55 tabular-nums sm:hidden">
                  {copy.updated} {lastUpdated.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                </p>
              )}
            </div>
          ...
          <div className="relative z-10 mt-6 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
            {executiveNotes.map((note, idx) => (
              <div 
                key={note.label} 
                className="group relative overflow-hidden rounded-[20px] border border-[var(--glass-border)] bg-[var(--glass-soft-bg)] p-4 transition-colors hover:bg-[var(--glass-strong-bg)] sm:p-5"
              >
                ...
                <div className="relative z-10 flex min-h-[7.5rem] flex-col justify-between">
                  ...
                  <div className="min-w-0 space-y-1">
                    ...
                    <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <span className="text-xs font-semibold uppercase tracking-wider text-primary/70">Live</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="px-4 space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="glass-panel rounded-[20px] p-5 glass-border animate-pulse">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="glass-panel glass-border h-80 rounded-[20px] p-8 animate-pulse" />
            ))}
          </div>
        </div>
      ) : error ? (
            ...
            <section className="grid grid-cols-1 xl:grid-cols-2 gap-4">
              {strongestStandard && (
                <div className="glass-panel glass-border rounded-[26px] p-5 sm:p-6 relative overflow-hidden">
                  ...
                  <div className="relative z-10 flex items-start justify-between gap-4">
                    <div>
                      <p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--status-success)]">{copy.strongestStandard}</p>
              ...
              {weakestStandard && (
                <div className="glass-panel glass-border rounded-[26px] p-5 sm:p-6 relative overflow-hidden">
                  ...
                  <div className="relative z-10 flex items-start justify-between gap-4">
                    <div>
                      <p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--status-critical)]">{copy.needsAttention}</p>
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-soft-bg)] px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-foreground transition-colors hover:bg-[var(--glass-strong-bg)]"
                    >
                      {copy.runAnalysis}
            ...
            <section className="glass-panel glass-border relative overflow-hidden rounded-[28px] p-5 sm:p-6 lg:p-7">
              ...
              <div className="relative z-10 flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-[0.16em]">{copy.unusualReportsSub}</p>
                </div>
              </div>
              <div className="relative z-10 space-y-3">
                {analytics.anomalies.map((anomaly: any) => (
                  <div key={anomaly.reportId} className="glass-button flex items-center gap-4 rounded-[18px] border border-[var(--glass-border)] bg-[var(--glass-soft-bg)] p-4 transition-colors hover:bg-[var(--glass-strong-bg)]">
                    <div className="w-11 h-11 rounded-2xl status-warning border flex items-center justify-center shrink-0 shadow-[0_18px_34px_-28px_rgba(180,83,9,0.45)]">
                      <span className="mono text-xs font-bold">{Math.round(anomaly.score)}</span>
                    </div>
