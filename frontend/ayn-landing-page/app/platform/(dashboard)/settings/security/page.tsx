    ...
    <div className="animate-fade-in-up pb-20 max-w-2xl px-4">
      ...
      <header className="mb-10">
        <h1 className="text-3xl font-black tracking-tight text-white">
          Security & <span className="text-zinc-600 font-light">Encryption</span>
        </h1>
      ...
      <div className="glass-panel p-6 rounded-2xl border-white/5 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <Label className="text-zinc-300 font-medium">Multi-Factor Authentication</Label>
            <p className="text-[11px] text-zinc-500 mt-0.5">Require MFA for account access</p>
          </div>
          <Switch checked={mfaEnabled} onCheckedChange={handleMfaToggle} />
        </div>
        <div>
          <Label className="text-zinc-300 font-medium">Session Timeout (hours)</Label>
          <div className="flex gap-2 mt-2">
            <input
              ...
              className="w-20 h-9 rounded-lg bg-white/5 border border-white/10 px-3 text-white text-sm"
            />
            <Button size="sm" variant="outline" onClick={handleSaveTimeout}>
              Save
            </Button>
          </div>
          <p className="text-[11px] text-zinc-500 mt-1">Auto-logout after inactivity</p>
        </div>
      </div>

      <div className="mt-8 glass-panel p-6 rounded-2xl border-emerald-500/20 bg-emerald-950/10">
