...
export function EditStandardPageClient({ standardId }: { standardId: string }) {
  ...
  useEffect(() => {
    if (standard) {
      ...
      setCategory(standard.category ?? "")
      setRegion(standard.region ?? "")
      setDescription(standard.description ?? "")
      setEstimatedSetup(standard.estimatedSetup ?? "")
      setColor(standard.color ?? COLOR_OPTIONS[0].value)
    }
  }, [standard])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true)
    try {
      await api.updateStandard(standardId, {
        title,
        description,
        code,
        category,
