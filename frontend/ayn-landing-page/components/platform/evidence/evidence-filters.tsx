    ...
    return (
        ...
        <div className="flex flex-col sm:flex-row gap-2.5">
            ...
            <div className="relative flex-1">
                <Search className="absolute start-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground pointer-events-none" />
                <input
                    type="text"
                    value={query}
                    onChange={(e) => handleSearch(e.target.value)}
                    placeholder={isArabic ? "ابحث بالعنوان أو اسم الملف…" : "Search by title or filename…"}
                    className="w-full ps-10 pe-4 py-2.5 border border-[var(--glass-border)] bg-white dark:bg-[var(--glass-soft-bg)] border-[var(--glass-border)] rounded-xl text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-1 focus:ring-primary/30 focus:border-primary/30 transition-all"
                />
                {query && (
                    <button
                        onClick={() => handleSearch("")}
                        className="absolute end-3 top-1/2 -translate-y-1/2 p-0.5 rounded text-muted-foreground hover:text-foreground transition-colors"
            ...
            <div className="flex flex-wrap gap-2 relative">
                ...
                <div className="relative">
                    <button
                        ...
                        className={cn(
                            "flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-black transition-all border",
                            activeStatus
                                ? "text-primary border-primary/40 bg-primary/10"
                                : "text-muted-foreground border-[var(--glass-border)] bg-white dark:bg-[var(--glass-soft-bg)] hover:border-primary/25 hover:text-foreground"
                        )}
                    >
                        ...
                        {activeStatus && (
                            <span
                                ...
                                className="ms-1 p-0.5 rounded-full hover:bg-primary/20 cursor-pointer"
                            >
                                <X className="w-3 h-3" />
                            </span>
                        )}
                    ...
                    {showStatusMenu && (
                        <div className="absolute top-full start-0 mt-2 z-30 rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-modal)]/95 shadow-2xl min-w-[160px] py-1.5 animate-in fade-in slide-in-from-top-2 duration-200 backdrop-blur-xl">
                            {STATUSES.map((s) => (
                                <button
                                    key={s}
                                    onClick={() => { onStatusChange(s); setShowStatusMenu(false) }}
                                    className={cn(
                                        "w-full text-start px-5 py-2.5 text-sm font-black hover:bg-[var(--surface-modal)] transition-colors first:rounded-t-2xl last:rounded-b-2xl",
                            ...
                            {!standards || standards.length === 0 ? (
                                ...
                                standards.map((s: any) => (
                                    <button
                                        ...
                                        className={cn(
                                            "w-full text-start px-5 py-3 text-sm hover:bg-[var(--surface-modal)] transition-colors first:rounded-t-2xl last:rounded-b-2xl group",
